...
public class PatientService : IPatientService
{
    ...
    public PatientService(ApplicationDbContext db, IAuditLogService audit, ICacheService cache)
    {
        _db = db; _audit = audit; _cache = cache;
    }

    private static PatientResponse ToResponse(Patient p) => new(
        p.Id, p.PatientNumber, p.FirstName, p.LastName, p.DateOfBirth, p.Gender,
        p.PhoneNumber, p.Email, p.InsuranceProvider, p.CreatedAtUtc);
    ...
    public async Task<PatientResponse> CreateAsync(CreatePatientRequest request, string actor, string ip)
    {
        ...
        await _audit.LogAsync(null, actor, "PATIENT_CREATE", "Patient", patient.Id.ToString(), ip);
        return ToResponse(patient);
    }

    public async Task<PatientResponse> UpdateAsync(Guid id, UpdatePatientRequest request, string actor, string ip)
    {
        var patient = await _db.Patients.FindAsync(id) ?? throw new KeyNotFoundException("Patient not found.");
        ...
        await _audit.LogAsync(null, actor, "PATIENT_UPDATE", "Patient", id.ToString(), ip);
        return ToResponse(patient);
    }
    ...
    {
        var patient = await _db.Patients.FindAsync(id) ?? throw new KeyNotFoundException("Patient not found.");
        patient.IsDeleted = true; // soft delete to preserve clinical history
        patient.UpdatedAtUtc = DateTime.UtcNow;
        await _db.SaveChangesAsync();
        await _cache.RemoveAsync($"patient:{id}");
        await _audit.LogAsync(null, actor, "PATIENT_DELETE", "Patient", id.ToString(), ip);
    }
    ...
    public async Task<PagedResult<PatientResponse>> SearchAsync(PatientSearchQuery query)
    {
        ...
        if (!string.IsNullOrWhiteSpace(query.Name))
            q = q.Where(p => (p.FirstName + " " + p.LastName).ToLower().Contains(query.Name.ToLower()));
        ...
        var items = await q.OrderBy(p => p.LastName)
            .Skip((query.Page - 1) * query.PageSize).Take(query.PageSize)
            .Select(p => new PatientResponse(p.Id, p.PatientNumber, p.FirstName, p.LastName, p.DateOfBirth,
                p.Gender, p.PhoneNumber, p.Email, p.InsuranceProvider, p.CreatedAtUtc))
            .ToListAsync();

        return new PagedResult<PatientResponse>(items, total, query.Page, query.PageSize);
    }
}
